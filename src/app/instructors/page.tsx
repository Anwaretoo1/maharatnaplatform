...
export default async function InstructorsPage() {
  ...
  const instructors = users.filter(user => user.role === 'craftsman');

  return (
    <main className="flex min-h-screen flex-col items-center p-8 md:p-24">
      <h1 className="text-4xl font-bold mb-12 text-center">المعلمون المبدعون</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-7xl">
        {instructors.map((instructor) => (
          <div key={instructor.id} className="flex flex-col items-center p-6 border rounded-lg shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-neutral-900 dark:border-neutral-800">
            <div className="w-32 h-32 rounded-full overflow-hidden mb-4 border-4 border-gray-100 dark:border-neutral-800">
              <img 
                src={instructor.avatar || instructor.bio && `https://ui-avatars.com/api/?name=${instructor.name}` || `https://ui-avatars.com/api/?name=${instructor.name}`} 
                alt={instructor.name} 
            ...
            <p className="text-center text-gray-600 dark:text-gray-400 text-sm mb-6 line-clamp-3">
              {instructor.bio || 'لا يوجد نبذة تعريفية'}
            </p>
      ...
      {instructors.length === 0 && (
        <div className="text-center py-12">
          <p className="text-xl text-gray-500">لا يوجد معلمون مسجلون حالياً.</p>
        </div>
      )}
    </main>
  );
